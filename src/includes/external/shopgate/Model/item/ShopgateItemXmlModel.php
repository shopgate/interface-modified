...
class ShopgateItemXmlModel extends ShopgateItemModel
{
    ...
    {
        $taxRate = (float)xtc_get_tax_rate(
            $this->item['products_tax_class_id'], $this->countryId,
            $this->config->getTaxZoneId()
        );
    ...
    {
        parent::setTaxClass(
            $this->getTaxClassTitleById($this->item['products_tax_class_id'])
        );
    }
    ...
    public function setDescription()
    {
        parent::setDescription(
            $this->getDescriptionToProduct(
                $this->item, $this->config->getExportDescriptionType()
            )
        );
    }
    ...
    {
        parent::setDeeplink(
            $this->generateDeepLinkToProduct(
                $this->item['products_id'], $this->item['products_name']
            )
        );
    }
    ...
    public function setPrice()
    {
        ...
        $xtPrice                                               = new xtcPrice(
            $this->config->getCurrency(), DEFAULT_CUSTOMERS_STATUS_ID_GUEST
        );
        ...
        if ($this->getIsChild()) {
            ...
            $salePrice       = $salePrice + ($additionalPrice
                    * $this->exchangeRate);
        }
    ...
    public function setManufacturer()
    {
        ...
        $uid     = isset($this->item['manufacturers_id'])
            ? $this->item['manufacturers_id'] : 0;
        $modelId = isset($this->item['products_manufacturers_model'])
            ? $this->item['products_manufacturers_model'] : '';
    ...
    {
        $result       = array();
        $crossSellIds = $this->getRelatedShopItems(
            $this->item['products_id'], true
        );

        if (!empty($crossSellIds)) {
            $crossSellRelation = new Shopgate_Model_Catalog_Relation();
            $crossSellRelation->setType(
    ...
    {
        if ($this->getVariationCombinationCount()
            ...
            foreach ($variations as $variationGroup) {
                ...
                foreach ($variationGroup as $variation) {
                    ...
                    $price = $variation['price_prefix'] == "-"
                        ...
                        : $priceHelper->formatPriceNumber(
                            $variation['options_values_price'], 2
                        );
                    $price = $price * (1 + $taxRate / 100);

                    $this->orderInfo["attribute_{$attributeCount}"][$variation['products_attributes_id']][]
                        = array(
                        'options_id'        => $variation['products_options_id'],
                        'options_values_id' => $variation['products_options_values_id'],
                    );
                    ...
                    if (
                        ...
                        || (!empty($optionAsInput)
                            && in_array(
                                $variation['products_options_id'],
                                $optionAsInput
                            ))
                    ) {
                        $input->setType(
                            Shopgate_Model_Catalog_Input::DEFAULT_INPUT_TYPE_TEXT
                        );
                        ...
                        $input->setType(
                            Shopgate_Model_Catalog_Input::DEFAULT_INPUT_TYPE_SELECT
                        );
            ...
            if (!empty($parentOrderInfo)) {
                if (!empty($this->orderInfo)) {
                    $orderInfo = array_merge(
                        $parentOrderInfo, $this->orderInfo
                    );
                    parent::setInternalOrderInfo($this->jsonEncode($orderInfo));
                }
            } else {
                if (!empty($this->orderInfo)) {
                    parent::setInternalOrderInfo(
                        $this->jsonEncode($this->orderInfo)
                    );
                }
            }
            ...
            foreach ($childInputs as $elements) {
                foreach ($elements as $childInput) {
                    $price = $childInput['price_prefix'] == "-"
                        ? $priceHelper->formatPriceNumber(
                            $childInput['options_values_price'] * (-1), 2
                        )
                        : $priceHelper->formatPriceNumber(
                            $childInput['options_values_price'], 2
                        );
                    $input = new Shopgate_Model_Catalog_Input();
    ...
    {
        parent::setDisplayType(
            Shopgate_Model_Catalog_Product::DISPLAY_TYPE_DEFAULT
        );
    }
        ...
        if ($this->getVariationCombinationCount()
            ...
            foreach ($this->cache['currentChild'] as $attribute) {
                ...
                if (!isset($parentAttGroups[$attribute['products_options_id']])) {
                    ...
                    $attributeGroupItem
                        = new Shopgate_Model_Catalog_AttributeGroup();
                    $attributeGroupItem->setUid(
                        $attribute['products_options_id']
                    );
                    $attributeGroupItem->setLabel(
                        $attribute['products_options_name']
                    );
                    $parentAttGroups[$attribute['products_options_id']]
                        = $attributeGroupItem;
                }
            }
    ...
    private function getVariationCombinationCount()
    {
        return (!empty($this->cache['variationCount']))
            ...
            : $this->cache['variationCount']
                = $this->calculateVariationAmountByOptions($this->getOptions());
    }
    ...
    private function getVariationInputs()
    {
        return $this->generateAttributes(
            $this->getAttributesInputFieldsToProductsQuery(
                $this->item['products_id']
            )
        );
    }
    ...
    /**
     * read all variations which are NOT of type "text field" to an products
     * from the database
     *
     * @return array|array[][]
     */
    ...
    {
        return $this->generateAttributes(
            $this->getAttributesToProductQuery($this->item['products_id'])
        );
    }
    ...
    private function getOptions()
    {
        return (!empty($this->cache['options']))
            ? $this->cache['options']
            : $this->cache['options'] = $this->generateAttributes(
                $this->getAllAttributesToProductsQuery(
                    $this->item['products_id']
                ), true
            );
    }
    ...
    {
        return trim(
            preg_replace('/<[^>]+>/', '', $this->item['products_name'])
        );
    }
    ...
    public function generateStockQuantity()
    {
        ...
        if (!empty($this->item['specials_new_products_price'])
            ...
            $qty = $this->item['specials_quantity']
    ...
    protected function addTierPricesTo(
         ...
         */
        foreach (
            $this->xtPricesByCustomerGroups as $customerGroupId => $xtPrice
        ) {
            ...
            $getDiscountsOnly
                = ($xtPrice->cStatus['customers_status_graduated_prices']
                == '1')
                ? ''
                : ' AND `quantity` = 1 ';
            ...
            $quantitiesQuery = xtc_db_query(
                ...
                . ((int)$customerGroupId) . '` ' .
                'WHERE `products_id` = ' . ((int)$this->item['products_id'])
                . ' ' . $getDiscountsOnly .
                'ORDER BY `quantity`;'
            );
            ...
            while ($quantity = xtc_db_fetch_array($quantitiesQuery)) {
                ...
                if ($addAmount > 0) {
                    $graduatedPrice += $addAmount -
                        (($xtPrice->cStatus['customers_status_discount_attributes'])
                            ? ($addAmount * ($xtPrice->xtcCheckDiscount(
                                        $this->item['products_id']
                                    ) / 100))
                            : 0);
                }
                ...
                $tierPriceModel->setReduction(
                    $priceHelper->formatPriceNumber($reduction, 6)
                );
                $tierPriceModel->setReductionType(
                    Shopgate_Model_Catalog_TierPrice::DEFAULT_TIER_PRICE_TYPE_FIXED
                );
                $tierPriceModel->setAggregateChildren(
                    true
            ...
            if ($xtPrice->cStatus['customers_status_ot_discount'] > 0) {
                ...
                $tierPriceModel->setReduction(
                    $priceHelper->formatPriceNumber($discount, 6)
                );
                $tierPriceModel->setReductionType(
                    Shopgate_Model_Catalog_TierPrice::DEFAULT_TIER_PRICE_TYPE_FIXED
                );
    ...
    protected function calculateVariationsAddAmount(xtcPrice $xtPrice)
    {
        ...
        foreach ($this->cache['currentChild'] as $variation) {
            $price           = $xtPrice->xtcGetOptionPrice(
                $this->item['products_id'], $variation['products_options_id'],
                $variation['products_options_values_id']
            );
